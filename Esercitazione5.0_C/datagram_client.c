...
int main(int argc, char **argv){
	...
	while (gets(req.nomefile)){
		/* invio richiesta */
		printf("Carattere da contare:\n");
		scanf("%c",&req.carattere);
		while((c=getchar())!= '\n'&& c!= EOF);
		len=sizeof(servaddr);
		...
		if (recvfrom(sd, &num_char, sizeof(num_char), 0, (struct sockaddr *)&servaddr, &len)<0){
			perror("recvfrom");
			printf("Nome del file:\n");
			continue; // se questa ricezione fallisce il client torna all'inzio del ciclo
		}
		if(num_char==-1)
			printf("File %s inesistenze\n",req.nomefile);
		else
			printf("Numero di carattericontenuto su file %s : %u\n",req.nomefile, ntohl(num_char));
		printf("Nome del file:\n");
