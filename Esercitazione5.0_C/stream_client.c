...
int main(int argc, char *argv[]){
	int sd, nread, port;
	char c_req,c, ok, nome_file[LENGTH_FILE_NAME];
	struct hostent *host;
	...
	while (gets(nome_file)){
		...
		printf("Nome del carattere da raddoppiare:\n");
		scanf("%c",&c_req);
				while((c=getchar())!= '\n'&& c!= EOF);
		if (write(sd, &c_req, sizeof(char))<0){
					perror("write");
					break;
				}
		printf("Richiesta del carattere %c inviata... \n", c_req);
		if (read(sd, &ok, 1)<0){
			perror("read");      
			break;
		}
