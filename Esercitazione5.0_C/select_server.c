//Nicola Sebastianelli 0000722894 Esercitazione 5
...
/********************************************************/
int conta_car (request req){
	int count = 0,fd_file,nread;
	char car;
	fd_file=open(req.nomefile, O_RDONLY);
	if (fd_file<0){
		printf("File inesistente\n");
		return -1;
	}
	else{
		while((nread=read(fd_file, &car, sizeof(char)))>0){
			if(car==req.carattere)
			count++;
		}
...
int main(int argc, char **argv){
	...
	const int on = 1;
	char zero=0, car, nome_file[LENGTH_FILE_NAME],c;
	fd_set rset;
	request req;
	...
	for(;;){
		...
		if (FD_ISSET(listenfd, &rset)){
			printf("Ricevuta richiesta di raddioppio di carattere in un file\n");
			...
			if (fork()==0){ /* processo figlio che serve la richiesta di operazione */
				...
				for (;;){
					...
					if ((read(connfd, &c, sizeof(char)))<=0)
					{ perror("read"); break; }
					printf("Carattere interessato %c\n", c);
					...
					if (fd_file<0){
						...
						while((nread=read(fd_file, &car, sizeof(char)))>0){
							if ((nwrite=write(connfd, &car, sizeof(char)))<0 )
							{perror("write"); break;}
							if(car==c)
							{
								if ((nwrite=write(connfd, &car, sizeof(char)))<0 )
		...
		if (FD_ISSET(udpfd, &rset)){
			printf("Server: ricevuta richiesta di conteggio carattere\n");
			len=sizeof(struct sockaddr_in);
			if (recvfrom(udpfd, &req, sizeof(req), 0, (struct sockaddr *)&cliaddr, &len)<0)
			{perror("recvfrom"); continue;}

			printf("Richiesto conteggio dei caratteri %c in %s\n",req.carattere, req.nomefile);
			num = conta_car(req);
